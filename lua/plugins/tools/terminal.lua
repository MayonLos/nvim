return {
	...
	config = function(_, opts)
		...
		map(
			{ "n", "t" },
			"<leader>tf",
			toggle_term("float", { direction = "float" }),
			vim.tbl_extend("force", kopt, { desc = "Terminal Float" })
		)
		map(
			{ "n", "t" },
			"<leader>tv",
			toggle_term("vertical", { direction = "vertical", size = 40 }),
			vim.tbl_extend("force", kopt, { desc = "Terminal Vertical" })
		)
		map(
			{ "n", "t" },
			"<leader>tt",
			toggle_term("split", { direction = "horizontal", size = 15 }),
			vim.tbl_extend("force", kopt, { desc = "Terminal Split" })
		)
		map(
			{ "n", "t" },
			"<leader>tg",
			toggle_term("lazygit", { direction = "float", cmd = "lazygit" }),
			vim.tbl_extend("force", kopt, { desc = "Terminal Lazygit" })
		)
